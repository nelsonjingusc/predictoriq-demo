...
import { useMockData, isDemoMode } from '@/lib/demo-mode';
import { mockAgentsFeedData } from '@/lib/mock-data';
...
export default function AgentsPage() {
  ...
  const { data: rawData, loading } = useMockData<AgentFeedResponse>(
    () => client.getAgentFeed(selectedAgent === 'all' ? undefined : selectedAgent),
    mockAgentsFeedData
  );

  // Filter mock data based on selected agent in demo mode
  const data = rawData && isDemoMode() && selectedAgent !== 'all'
    ? {
      ...rawData,
      messages: rawData.messages.filter(m => m.agent === selectedAgent),
      agent_filter: selectedAgent as any,
    }
    : rawData;
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex gap-3 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedAgent === 'all'
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedAgent === 'alpha_scout'
              ? 'bg-blue-600 text-white'
              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedAgent === 'portfolio_guardian'
              ? 'bg-blue-600 text-white'
              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedAgent === 'research_autopilot'
              ? 'bg-blue-600 text-white'
              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
          >
        ...
        <div className="space-y-4">
          {data.messages.map((message, idx) => (
            <div
              key={idx}
              className={`bg-white rounded-lg shadow-sm p-5 border-l-4 ${message.priority === 'high' ? 'border-red-500' :
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`px-2 py-1 rounded text-xs font-medium ${message.priority === 'high' ? 'bg-red-100 text-red-700' :
